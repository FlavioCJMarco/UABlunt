...
def facultadCiencias(m):
    ...
    if (sitShown[cid] == False):
        ...
        if (caladas[cid] >= 25):
            ...
            time.sleep(2)
            bot.send_message(cid, 'El hombre se gira y resulta ser Albert Einstein:')
            time.sleep(2)
            ...
            if boolMuerte[cid] == False:
                ...
                time.sleep(2)
                bot.send_message(cid,
                                 '"¡Mi amada! Siempre hay alguien que antepone el beneficio propio aunque eso les cueste su integridad. Ya lo sabes. ¿Sospechas que todo lo que está pasando en la universidad está relacionado con algún ingeniero? ¿Estás segura? Sería la tercera vez que nos vemos afectados por algo así."')
                time.sleep(3)
                ...
                time.sleep(2)
                bot.send_message(cid, '¿Paloma? Un poco de respeto, está dirigiéndose a la señora Tesla.')
                time.sleep(2)
                ...
                time.sleep(2)
                bot.send_message(cid, '"No pasa nada. ¿Necesita algo?, estaba manteniendo una conversación privada.", te dice',
                                 reply_markup=dialog)
                while (dialogo[cid] == False):
                    pass
                dialogo[cid] = False
                bot.send_message(cid, 'Contestas: -"Me gustaría conectarme a una red WIFI¿Sabe de algún lugar donde funcione?"')
                time.sleep(2)
                bot.send_message(cid,
                                 'No sé de qué me estás hablando. Pero tal vez el hombre calvo ese que siempre viste igual, de negro y un tejano azul pueda ayudarte.',
                                 reply_markup=dialog)
                while (dialogo[cid] == False):
                    pass
                dialogo[cid] = False
                ...
                time.sleep(2)
                bot.send_message(cid, 'Gracias y… vaya al hospital. Eso es una paloma.')
                time.sleep(2)
                bot.send_message(cid,
        ...
        if boolMuerte[cid] == False:
            ...
            if boolMuerte[cid] == False:
                if objeto3[cid] == 1:
                    ...
                    bot.send_message(cid, 'Sacas el martillo y empiezas a golpear la puerta.', reply_markup=game_menu)
                    time.sleep(2)
                    bot.send_message(cid,
    ...
    if (gameON[cid] == True):
        ...
        if sit3bool[cid] == False:
            if data[cid] == '3B' and gameON[cid] == True:
                ...
                time.sleep(2)
                bot.send_message(cid,
                                 "Por fin, te encuentras a alguien que conoces, es tu amigo Jorge, que estudia el proceso de cómo tragan las jirafas allí en Veterinaria, siempre ha sido cuanto menos curioso, el chaval.")
                time.sleep(2)
                ...
                time.sleep(2)
                bot.send_message(cid,
                                 "De repente el cielo cambia de color y tu amigo Jorge empieza a hacer cosas raras. Se está transformando... ¡en una jirafa! O en algo parecido... Creo que estás fumao.\n\nSe ha añadido una calada al contador de caladas. Puedes acceder a él a través de la opción 'STATS 📖'")
                time.sleep(2)
                sit3bool[cid] = True
        ...
        if sit27bool[cid] == False:
            if data[cid] == '27A':
                bot.send_message(cid,
                                 'Ves como Goku se dirige a los de la puerta y se lía a ostia limpia con ellos. Escuchas algún que otro Kame Hame Ha.')
                time.sleep(2)
        ...
        if sit29bool[cid] == False:
            if data[cid] == '29A':
                ...
                time.sleep(2)
                bot.send_message(cid, 'Exhalando el humo, sales de allí como el antihéroe que eres.')
                time.sleep(2)
                userStep[cid] = 0
                sit29bool[cid] = True
                set_game_bools(cid)
